...
import { BrainCircuit, Clock3, Filter, Loader2, PencilLine, Plus, Save, Search, Tag, Trash2 } from 'lucide-react';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const selectClassName = 'field-surface flex h-11 w-full rounded-2xl border-white/6 bg-white/[0.02] px-3.5 text-sm capitalize text-white/92 shadow-none outline-none transition focus:border-white/12 focus:bg-white/[0.028] focus:ring-2 focus:ring-white/10 [&>option]:bg-[hsl(0,0%,10%)] [&>option]:text-white';
...
const badgeClassName = 'rounded-full border border-white/6 bg-white/[0.025] px-3 py-1 text-[11px] tracking-[0.02em] text-white/62';
const surfaceLabelClassName = 'mb-4 flex items-center gap-2 text-[11px] tracking-[0.08em] text-white/72';
...
export const MemoryDialog = ({ open, onOpenChange }: { open: boolean, onOpenChange: (o: boolean) => void }) => {
            ...
            <DialogContent className="glass-panel flex max-h-[calc(100dvh-1.5rem)] w-[min(96vw,84rem)] max-w-7xl flex-col overflow-hidden p-0 text-foreground">
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div className="space-y-3">
                            ...
                            <DialogDescription className="max-w-3xl text-sm leading-6 text-white/62 sm:text-[15px]">
                                Bellek kayıtlarını arayın, filtreleyin ve düzenleyin. Yardımcı kontroller ve liste görünümü tek bir daha sakin panel hiyerarşisine bağlandı.
                            </DialogDescription>
                        </div>
                        <div className="flex flex-wrap gap-2 lg:max-w-md lg:justify-end">
                            <span className={badgeClassName}>{filteredMemories.length} görünür kayıt</span>
                            <span className={badgeClassName}>{editorOpen ? 'Düzenleme açık' : 'Liste görünümü'}</span>
                        </div>
                    </div>
                </DialogHeader>
                ...
                <div className="grid min-h-0 flex-1 gap-0 xl:grid-cols-[368px_minmax(0,1fr)]">
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="section-surface rounded-[22px] p-4">
                                        <div className="text-[11px] tracking-[0.08em] text-white/72">Toplam</div>
                                        <div className="mt-2 text-2xl font-semibold tracking-tight text-foreground">{memories.length}</div>
                                    </div>
                                    <div className="section-surface rounded-[22px] p-4">
                                        <div className="text-[11px] tracking-[0.08em] text-white/72">Filtrelenen</div>
                                        <div className="mt-2 text-2xl font-semibold tracking-tight text-foreground">{filteredMemories.length}</div>
                                    </div>
                                </div>

                                <div className="section-surface rounded-[26px] p-5">
                                    <div className="mb-3 text-[11px] tracking-[0.08em] text-white/72">Kategori yoğunluğu</div>
                                ...
                                <div className="section-surface rounded-[26px] p-5">
                                    <div className="mb-4 flex items-center justify-between gap-2">
                                        <div className="text-[11px] tracking-[0.08em] text-white/72">
                                            {editor.id ? 'Belleği düzenle' : 'Yeni bellek oluştur'}
                                        </div>
                                        <span className="rounded-full border border-white/6 bg-white/[0.025] px-2.5 py-1 text-[10px] tracking-[0.08em] text-white/75">
                    ...
                    <div className="subtle-scrollbar min-h-0 overflow-y-auto bg-gradient-to-b from-white/[0.01] to-transparent px-5 py-5 sm:px-6">
                        <div className="mb-5 flex flex-wrap items-center gap-2 text-[11px] tracking-[0.08em] text-white/72">
                                            ...
                                            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/6 pb-4">
                                                <div className="space-y-3">
                                                    <div className="flex flex-wrap items-center gap-2">
                                                        <span className="inline-flex items-center gap-1.5 rounded-full border border-white/6 bg-white/[0.025] px-3 py-1 text-[11px] tracking-[0.08em] text-white/62">
                                                            <Tag className="h-3.5 w-3.5" />
                                                            {memory.category || 'general'}
                                                        </span>
                                                        <span className={`inline-flex rounded-full px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] ${getImportanceTone(importance)}`}>
                                                            önem {importance}/10
                                                        </span>
                                                    </div>
